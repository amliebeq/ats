...
export const List = ({ list }) => {
    ...
    return (
        <ul className="border-black border-4 p-8">
            {edit ? <EditList edit={edit} setEdit={setEdit} list={list} /> : <a href={`/lists/${list.id}`} target="_blank" rel="noopener noreferrer" className="font-medium text-blue-600 dark:text-blue-500 hover:underline">Name: {list.name}</a>}
