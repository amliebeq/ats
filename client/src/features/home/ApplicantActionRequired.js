...
export const ApplicantActionRequired = () => {
    ...
    const renderApplicants = newApplicantList.map((applicant) => {
        return (
            <div>
                <Link key={applicant.id} to={`/candidates/${applicant.id}`} className="font-medium text-blue-600 dark:text-blue-500 hover:underline">{applicant.first_name} {applicant.last_name}</Link>
            </div>
    )})
